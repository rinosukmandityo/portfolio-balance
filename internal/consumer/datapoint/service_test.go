...
func TestService_DataPointConsumer_Success(t *testing.T) {
	t.Parallel()
	...
	expWriteMsg := newInt32Cnt(0)
	writeMsg := newInt32Cnt(0)
	...
	pendingWriter := &mock.ClientWriterMock{
	...
	svc, err := datapoint.NewService(
		...
		portfolioRepo,
		pendingWriter,
		datapoint.WithConsumersCount(1),
		datapoint.WithBucketSize(int(bucketSize)),
		datapoint.WithBucketWaitTime(waitTime),
	...
	require.GreaterOrEqual(t, writeMsg.value(), expWriteMsg.value())
}

func TestService_DataPointConsumer_Duplicate_Success(t *testing.T) {
	t.Parallel()
	...
	expTotalInsert := newInt32Cnt(totalBulkInsert)
	expAckCount := newInt32Cnt(totalData)
	expWriteMsg := newInt32Cnt(totalBulkInsert)
	writeMsg := newInt32Cnt(0)
	ctx, cancel := context.WithCancel(context.Background())
	logger := zap.NewNop()
	...
	svc, err := datapoint.NewService(
		...
		datapoint.WithMsgTotalMetric(monitoring.ConsumeTotal),
		datapoint.WithMsgLagDurationMetric(monitoring.ConsumeLagSeconds),
		datapoint.WithInsertPending(true),
	)
