...
// WithMsgLagDurationMetric configures message consumed lag duration metric.
func WithMsgLagDurationMetric(metric metrics.Histogram) Option {
	return func(svc *Service) {
		svc.metricMsgLagDuration = metric
	}
}
...
func WithMessageRateLimit(rps float64, burst int) Option {
	return func(svc *Service) {
		if rps < 0 {
		...
		svc.messageBurst = burst
	}
}

// WithInsertPending configures insert pending.
func WithInsertPending(enabled bool) Option {
	return func(svc *Service) {
		svc.enableInsertPending = enabled
	}
}
